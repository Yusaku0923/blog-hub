...
### I. consumer 側の `components: [{ path: '~/components', pathPrefix: false }]` は **必須**（推奨ではない）

§G では「consumer の `components:` 宣言は冗長だが残した」と書いたが、実際は **必須** だった。M3 の `blog-site-template` を install→build した際、`components/home/Hero.vue` が `<Hero>` ではなく `<HomeHero>` として解決されようとして失敗。

**原因**: layer の `components: [{ path: resolve(layerDir, 'components'), pathPrefix: false }]` 設定は**layer 自身のコンポーネントにのみ**適用される。consumer の `~/components/` 配下のサブディレクトリ（`layout/`, `mobile/`, `home/` 等）は別ルートとして登録され、デフォルトの `pathPrefix: true` で名前にプレフィックスが付く。

**採用した解**: consumer の `nuxt.config.ts` に明示的に書く:
```ts
components: [
...
**含意**: §G を修正 — `modules:` は冗長（消してもよい）だが、`components:` は **subdir があるサイトでは必ず書く**。

### J. `better-sqlite3` を consumer の devDep に必要

`@nuxt/content` v3 はビルド時に SQLite バックエンド（`better-sqlite3`）を使う。layer は peerDeps に含めていない（実行時には不要なため）が、consumer 側で `pnpm install` すると `nuxt prepare` が SQLite アダプタの欠落で失敗する。

**採用した解**: consumer の `package.json` の devDependencies に `"better-sqlite3": "^12.9.0"` を追加（PetGurashi も同じ依存を持っている）。

**含意**: 新サイトテンプレ（`blog-site-template`）は最初から `better-sqlite3` を含む。layer 側で peerDep に格上げする選択肢もあるが、現状は consumer 側で持つ運用。
